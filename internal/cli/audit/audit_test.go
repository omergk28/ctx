...
// newAuditCmd builds a fresh audit command with the given args.
...
func TestShow_UnknownID(t *testing.T) {
	setup(t)
	_, err := runCmd(newAuditCmd("show", "ghost"))
	if err == nil {
		t.Fatal("expected error for unknown id")
	}
	if !strings.Contains(err.Error(), "unknown audit id") {
		t.Errorf("err = %q, want 'unknown audit id'", err.Error())
	}
}
...
func TestDismiss_StopsRelay(t *testing.T) {
	...
	reports, _ := store.Read()
	led, _ := store.ReadDismissals()
	if len(reports) != 1 {
...
func TestDismiss_NoIDsNoAll_Errors(t *testing.T) {
	setup(t)
	_, err := runCmd(newAuditCmd("dismiss"))
	if err == nil {
		t.Fatal("expected error when no ids and no --all")
	}
	if !strings.Contains(err.Error(), "audit id required") {
		t.Errorf("err = %q, want 'audit id required'", err.Error())
	}
}
