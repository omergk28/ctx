...
func TestRun_NoLeakInUninitializedProject(t *testing.T) {
	...
	go func() {
		...
		if _, copyErr := io.Copy(w, bytes.NewReader([]byte(
	...
	if err := checkaudit.Run(cmd, r, relayStrings()); err != nil {
...
func TestRun_SilentOnCleanReports(t *testing.T) {
	...
	dropRelayReport(t, tempDir, "surface", cfgAudit.StatusClean, "no findings")
...
func TestRun_RelaysFindingsBody(t *testing.T) {
	...
	dropRelayReport(t, tempDir, "surface", cfgAudit.StatusFindings, body)
	...
	if !bytes.Contains(out, []byte("ctxctl audit dismiss")) {
...
func TestRun_StalePrefixOnOldReport(t *testing.T) {
	tempDir := setupInitializedProject(t)
	// generated-at in dropRelayReport's fixture is 2099;
	// to force staleness independent of wall clock, drop a
	// much older report.
...
// dropRelayReport writes a fresh-timestamp findings report.
func dropRelayReport(
	...
	// production but stable for the test.
	content := "---\n" +
		"kind: " + id + "\nstatus: " + status + "\n" +
...
func runHook(t *testing.T) ([]byte, error) {
	...
	cmd.SetOut(&buf)
	cmd.SetErr(&buf)
	err := checkaudit.Run(cmd, r, relayStrings())
